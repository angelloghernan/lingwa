...
//package.opf
public class Package extends com.example.lingwa.util.epubparser.BaseFindings {
	...
	public class Metadata {
		...
		void fillAttributes(NodeList nodeList) throws ReadingException {
					...
					if (nodeName.equals(fields[j].getName())) {
						...
						if (fields[j].getName().equals("subject")) {
							...
							try {
								...
								e.printStackTrace();
								throw new ReadingException("Exception while parsing " + com.example.lingwa.util.epubparser.Constants.EXTENSION_OPF + " content: " + e.getMessage());
							}
						}
					}
				}
			...
			if (subjectList != null) {
				Field field;
				try {
					field = Metadata.class.getDeclaredField("subject");
					...
					e.printStackTrace();
					throw new ReadingException("Exception while parsing subjects " + com.example.lingwa.util.epubparser.Constants.EXTENSION_OPF + " content: " + e.getMessage());
				}
			}
		}
